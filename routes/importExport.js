// routes/importExport.js — 匯入 / 匯出 API v16
'use strict';
...
router.get('/export/products', (req, res) => {
  try {
    const db = getDb();
    const products = db.all('SELECT * FROM products ORDER BY sort_order ASC, id ASC');
    ...
    const rows = products.map(p => ({
      ...
      '低庫存警戒(份)': p.low_stock_alert || 5,
      '商品狀態': p.sale_status || 'available',
      ...
      '優惠標籤': p.line_promo || 0, '今日完售': p.line_sold_out || 0,
      '商品圖片URL': p.image || '',
    }));
...
router.get('/export/product-inventory', (req, res) => {
  try {
    ...
    const rows = products.map(p => {
      ...
      let stockG = p.current_stock_grams || 0;
      let units = 0;
      let ingControlled = '否';
      if (formulas.length > 0) {
        ...
        let minU = Infinity;
        formulas.forEach(f => {
          const rG = toGrams(Number(f.refrigerated_stock||0), f.ing_unit||'g');
          const pu = Number(f.amount_per_unit||0);
          const u  = pu > 0 ? Math.floor(rG/pu) : 0;
          if (u < minU) { minU = u; stockG = rG; }
        });
        units = minU === Infinity ? 0 : minU;
      } else {
        const alloc = Number(p.allocated_grams||0);
        units = alloc > 0 ? Math.floor(stockG/alloc) : 0;
      }
      return {
        '商品名稱': p.name, '每份克數': p.allocated_grams||'',
        '目前庫存(g)': Number(stockG).toFixed(2), '可售數量': units,
        '低庫存警戒(份)': p.low_stock_alert||5, '食材控管': ingControlled,
      };
    });
    const csv = BOM + toCsv(headers, rows);
    res.setHeader('Content-Type', 'text/csv; charset=utf-8');
    res.setHeader('Content-Disposition', 'attachment; filename="product_inventory_export.csv"');
    res.send(csv);
  } catch(e) { res.status(500).json({ success: false, message: e.message }); }
});
...
router.get('/export/ingredients', (req, res) => {
  try {
    const db = getDb();
    const ings = db.all('SELECT * FROM ingredients ORDER BY id ASC');
    const headers = ['食材名稱','單位','冷凍庫存','解凍中','冷藏可販售','總庫存','低庫存警戒值','預設解凍時間(小時)','備註'];
    const rows = ings.map(i => ({
      '食材名稱': i.name, '單位': i.unit,
      '冷凍庫存': i.frozen_stock||0, '解凍中': i.thawing_stock||0,
      '冷藏可販售': i.refrigerated_stock||0, '總庫存': i.total_stock||0,
      '低庫存警戒值': i.low_stock_threshold||0, '預設解凍時間(小時)': i.default_thaw_hours||0,
      '備註': i.notes||'',
    }));
    const csv = BOM + toCsv(headers, rows);
    res.setHeader('Content-Type', 'text/csv; charset=utf-8');
...
router.get('/export/ingredient-formulas', (req, res) => {
  try {
    ...
    const rows = formulas.map(f => ({
      '商品名稱': f.product_name||'', '食材名稱': f.ingredient_name||'',
      '每份扣除量(g)': f.amount_per_unit||0,
    }));
    const csv = BOM + toCsv(headers, rows);
    res.setHeader('Content-Type', 'text/csv; charset=utf-8');
    res.setHeader('Content-Disposition', 'attachment; filename="ingredient_formulas_export.csv"');
    res.send(csv);
  } catch(e) { res.status(500).json({ success: false, message: e.message }); }
});

// ════════════════════════════════════════════════════════
//  IMPORT  (接收 JSON 陣列 — 前端解析 CSV 後送來)
// ════════════════════════════════════════════════════════
...
router.post('/import/products', (req, res) => {
  try {
    ...
    if (!Array.isArray(rows) || !rows.length)
      return res.status(400).json({ success: false, message: 'rows 必填' });
    ...
    rows.forEach((r, idx) => {
      try {
        ...
        // 分類不存在自動建立
        const catRow = db.get('SELECT id FROM categories WHERE name=?', [cat]);
        let catId = catRow ? Number(catRow.id) : 0;
        if (!catRow) {
          const r2 = db.run('INSERT OR IGNORE INTO categories (name,icon,sort_order,is_active) VALUES (?,?,99,1)', [cat,'📌']);
          catId = Number(r2.lastInsertRowid) || 0;
          if (!catId) { const c2 = db.get('SELECT id FROM categories WHERE name=?',[cat]); catId = c2 ? Number(c2.id) : 0; }
        }

        const existing = db.get('SELECT id FROM products WHERE name=?', [name]);
        if (existing) {
          db.run(`UPDATE products SET category=?,category_id=?,price=?,
            allocated_grams=?,low_stock_alert=?,
            show_on_line=?,line_price=?,line_description=?,line_image_url=?,
            line_category=?,line_hot=?,line_promo=?,line_sold_out=?,image=?,
            updated_at=datetime('now','localtime') WHERE id=?`,
            [cat, catId, price,
             parseFloat(r['每份克數']||0)||0, parseInt(r['低庫存警戒(份)']||5)||5,
             parseInt(r['LINE是否上架']??1), parseFloat(r['LINE售價']||0)||0,
             r['LINE描述']||'', r['LINE圖片URL']||'',
             r['LINE顯示分類']||'', parseInt(r['熱銷標籤']||0)||0,
             parseInt(r['優惠標籤']||0)||0, parseInt(r['今日完售']||0)||0,
             r['商品圖片URL']||'', existing.id]);
          updated++;
        } else {
          db.run(`INSERT INTO products (name,category,category_id,price,allocated_grams,low_stock_alert,show_on_line,line_price,line_description,line_image_url,line_category,line_hot,line_promo,line_sold_out,image) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
            [name, cat, catId, price,
             parseFloat(r['每份克數']||0)||0, parseInt(r['低庫存警戒(份)']||5)||5,
             parseInt(r['LINE是否上架']??1), parseFloat(r['LINE售價']||0)||0,
             r['LINE描述']||'', r['LINE圖片URL']||'',
             r['LINE顯示分類']||'', parseInt(r['熱銷標籤']||0)||0,
  ...
  try {
    ...
    rows.forEach((r, idx) => {
      try {
        const name = (r['商品名稱']||'').trim();
        if (!name) { errors.push(`第${idx+2}行：商品名稱不可空白`); failed++; return; }
        const prod = db.get('SELECT * FROM products WHERE name=?', [name]);
        ...
        db.run(`UPDATE products SET allocated_grams=?,current_stock_grams=?,low_stock_alert=?,inventory_enabled=1,updated_at=datetime('now','localtime') WHERE id=?`,
          [allocG, newStock, lowAlert, prod.id]);
        updated++;
      } catch(e) { errors.push(`第${idx+2}行：${e.message}`); failed++; }
    });
  ...
  try {
    ...
    rows.forEach((r, idx) => {
      try {
        ...
        const total = frozen;
        const existing = db.get('SELECT id FROM ingredients WHERE name=?', [name]);
        if (existing) {
          db.run(`UPDATE ingredients SET unit=?,low_stock_threshold=?,default_thaw_hours=?,notes=?,updated_at=datetime('now','localtime') WHERE id=?`,
            [unit, threshold, thawHours, notes, existing.id]);
          updated++;
        } else {
          db.run(`INSERT INTO ingredients (name,unit,frozen_stock,total_stock,low_stock_threshold,default_thaw_hours,notes) VALUES (?,?,?,?,?,?,?)`,
            [name, unit, frozen, total, threshold, thawHours, notes]);
          added++;
        }
      } catch(e) { errors.push(`第${idx+2}行：${e.message}`); failed++; }
    });
...
router.post('/import/ingredient-formulas', (req, res) => {
  try {
    ...
    rows.forEach((r, idx) => {
      try {
        const pName = (r['商品名稱']||'').trim();
        const iName = (r['食材名稱']||'').trim();
        ...
        const prod = db.get('SELECT id FROM products WHERE name=?', [pName]);
        const ing  = db.get('SELECT id FROM ingredients WHERE name=?', [iName]);
