...
router.get('/:id', (req, res) => {
  try {
    const db = getDb();
    const c = db.get('SELECT * FROM customers WHERE id=? OR phone=?', [req.params.id, req.params.id]);
    if (!c) return res.status(404).json({ success: false, message: '會員不存在' });
    const orders = db.all('SELECT * FROM orders WHERE customer_phone=? ORDER BY created_at DESC LIMIT 20', [c.phone]);
...
router.post('/', (req, res) => {
  try {
    ...
    try {
      db.run('INSERT INTO customers (name,phone,line_id) VALUES (?,?,?)', [name, phone, line_id]);
      res.status(201).json({ success: true, data: db.get('SELECT * FROM customers WHERE phone=?', [phone]) });
