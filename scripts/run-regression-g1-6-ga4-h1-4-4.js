...
// array). The parseH143FinalSuite() readFileSync+new Function() executor is
...
// Final uniqueness gate.
{
  ...
  if (dups.length) {
    throw new Error(`[FATAL] H1.4.4 runner 組出來的最終 SUITE 清單有重複 path：${JSON.stringify(dups)}`);
  }
}
...
function main() {
  ...
  const { tmpRoot, cleanupRoot } = dbHelper.createOrchestratorTempRoot('regression-ga4-h1-4-4');
  let rounds;
  try {
    rounds = [];
